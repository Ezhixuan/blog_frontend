<template>
        ...
        >
          ...
          <div class="cover-upload-container">
            ...
            <div v-if="!coverUrl" class="cover-upload-placeholder">
              <button
                type="button"
                @click="() => showPictureModal(PICTURE_TYPES.COVER)"
  ...
  <div>
    ...
    >
      <div v-if="contentExpanded" class="editor-content-container">
        <MarkdownEditor
          ref="markdownEditorRef"
          v-model="content"
          :title="title"
          :is-generating="isGenerating"
          @select-image="handleSelectImageForMarkdown"
...
onMounted(() => {
  ...
  // 添加自定义事件监听器
  const handleInsertImageToMarkdown = (event: CustomEvent) => {
    if (markdownEditorRef.value && markdownEditorRef.value.insertImage) {
      const { url, altText } = event.detail;
      markdownEditorRef.value.insertImage(url, altText);
    }
  };
  
  window.addEventListener('insertImageToMarkdown', handleInsertImageToMarkdown as EventListener);
  
  // 保存清理函数的引用
  (window as any).__insertImageHandler = handleInsertImageToMarkdown;
});

onUnmounted(() => {
  // 清理自定义事件监听器
  if ((window as any).__insertImageHandler) {
    window.removeEventListener('insertImageToMarkdown', (window as any).__insertImageHandler as EventListener);
    delete (window as any).__insertImageHandler;
  }
});
</script>
...
@media (max-width: 768px) {
  .editor-content-container {
    margin: 0 -1rem; /* 抵消父容器的padding */
    padding: 0 1rem;
    box-sizing: border-box;
  }
  
  /* 确保编辑器在移动端不会溢出 */
  .editor-content-container :deep(.md-editor) {
    max-width: 100% !important;
    overflow-x: hidden !important;
  }
  
  .editor-content-container :deep(.md-editor-v3) {
    max-width: 100% !important;
    overflow-x: hidden !important;
  }
  
  /* 编辑器工具栏在移动端的优化 */
  .editor-content-container :deep(.md-editor-toolbar-wrapper) {
    flex-wrap: wrap !important;
    overflow-x: auto !important;
  }
  
  /* 编辑器内容区域优化 */
  .editor-content-container :deep(.md-editor-content-wrapper) {
    overflow-x: hidden !important;
  }
  
  /* 预览区域优化 */
  .editor-content-container :deep(.md-editor-preview) {
    overflow-x: hidden !important;
    word-wrap: break-word !important;
  }
  
  /* 编辑区域优化 */
  .editor-content-container :deep(.md-editor-input-wrapper) {
    overflow-x: hidden !important;
  }
  
  /* 编辑器文本域优化 */
  .editor-content-container :deep(.md-editor-input) {
    overflow-x: hidden !important;
    word-wrap: break-word !important;
  }
}
